...
\chapter{Operating the Ground Control Software}
	...
	\section{Configuring the Payload}
		\subsection{Center Frequency, Sampling Frequency, and SDR Gain}
			...
			\begin{enumerate}
				...
				\item The current values of the payload's center frequency, sampling frequency, and SDR gain are displayed. Edit any values to the desired values. Refer to table ~\ref{tab:syssettings} for more information about these values.
		...
		\subsection{Expert and Debug Configuration}
			...
			\begin{enumerate}
				...
				\item In the resulting pop-up window (titled ``Expert/Engineering Settings''), enter the new desired values for the fields displayed. To learn about what these fields mean, refer to table ~\ref{tab:expertsettings}.
	...
	\section{Checking on the Payload Status}
		...
		\end{figure}
		The payload has five possible states (refer to table ~\ref{tab:sysstates} to learn about these states): 
		\begin{table}[htb]
			\centering
		...
		\begin{enumerate}
			...
			\item The entry next to ``Status'' is the current status of the payload. Refer to table ~\ref{tab:sysstates} to interpret this status.
		...
		\section{Checking the Status of Individual Components Within Payload}
			...
			\begin{enumerate}
				...
				\item Five individual component statuses will be displayed here. Refer to tables ~\ref{tab:sdrstates} through ~\ref{tab:swstates} to interpret these statuses.
			\end{enumerate}
